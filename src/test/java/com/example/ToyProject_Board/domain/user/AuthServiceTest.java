...
class AuthServiceTest {
    ...
    void 토큰_재발급_성공() {
        ...
        // when
        TokenResponse response = authService.reissue("valid_refresh_token");
    ...
    void 저장된_토큰과_불일치로_재발급_실패() {
        ...
        // when & then
        assertThatThrownBy(() -> authService.reissue("other_token"))
                .isInstanceOf(RuntimeException.class)
    ...
    void 회원가입_승인_성공() {
        ...
        User requested = UserFixture.createWithId(2L);
        given(userRepository.findById(1L)).willReturn(Optional.of(admin));
        given(userRepository.findById(2L)).willReturn(Optional.of(requested));
    ...
    @Test
    @DisplayName("회원가입 승인 실패 - 관리자 권한 없음")
    void 관리자가_아니면_회원가입_승인_실패() {
        // given
        User notAdmin = UserFixture.createWithId(1L);
        given(userRepository.findById(1L)).willReturn(Optional.of(notAdmin));

        // when & then
        assertThatThrownBy(() -> authService.approve(1L, 2L))
                .isInstanceOf(RuntimeException.class)
    ...
    @Test
    @DisplayName("회원가입 거절 실패 - 관리자 권한 없음")
    void 관리자가_아니면_회원가입_거절_실패() {
        // given
        User notAdmin = UserFixture.createWithId(1L);
        given(userRepository.findById(1L)).willReturn(Optional.of(notAdmin));

        // when & then
        assertThatThrownBy(() -> authService.reject(1L, 2L))
                .isInstanceOf(RuntimeException.class)
                .hasMessage("관리자 권한이 필요합니다");
    }
}